        ...
        {mapMode ? (
          <View style={styles.map}>
            ...
            {results.map((entry, index) => (
              ...
              >
                <SemanticIcon name="location" size={30} color={palette.accentStrong} />
              </Pressable>
            ))}
          </View>
        ) : null}
...
const createStyles = (palette: typeof AppPalette.light | typeof AppPalette.dark) => StyleSheet.create({
  ...
  search: { flex: 1, height: 38, flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 10, backgroundColor: palette.cardMuted, paddingHorizontal: Spacing.sm },
  query: { fontSize: Typography.fontSize.md, color: palette.text },
  ...
  map: { height: 280, borderRadius: 20, backgroundColor: palette.cardMuted, overflow: 'hidden' },
  road: { position: 'absolute', width: '120%', height: 13, backgroundColor: palette.card, top: '50%', left: '-10%', transform: [{ rotate: '-18deg' }] },
  pin: { position: 'absolute' },
  results: { gap: Spacing.md },
  card: { padding: Spacing.md },
  title: { fontSize: Typography.fontSize.md, fontWeight: '700', color: palette.text },
  subtitle: { marginTop: 4, fontSize: Typography.fontSize.sm, color: palette.secondaryText },
});
